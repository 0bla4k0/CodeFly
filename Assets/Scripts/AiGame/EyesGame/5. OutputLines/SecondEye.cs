...
public class SecondEye : MonoBehaviour  // change script name
{
    ...
    void Start()
    {
        ...
        line = new GameObject("LineOutSecond").AddComponent<LineRenderer>(); // change line name
    ...
    void Update()
    {
        if (SecondItemSlot.SetInRightBoxSecondEye) // change
        {
            ...
            {
                if (MouseDragStart == false && OutputBlock.SConnectLineEnter == false) // change
                {
                    line.SetPosition(0, StartBoxBlock.transform.position);
        ...
        if (OutputBlock.SConnectLineEnter) // change
        {
            mousePos = Camera.main.ScreenToWorldPoint(Input.mousePosition);
