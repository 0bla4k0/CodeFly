...
public class HealthyEye : MonoBehaviour  // change script name
{
    ...
    void Start()
    {
        ...
        line = new GameObject("LineOutHealthy").AddComponent<LineRenderer>(); // change line name
    ...
    void Update()
    {
        if (OneItemSlot.SetInRightBoxHealthyEye) // change
        {
            ...
            {
                if (MouseDragStart == false && OutputBlock.HConnectLineEnter == false) // change
                {
                    line.SetPosition(0, StartBoxBlock.transform.position);
        ...
        if (OutputBlock.HConnectLineEnter) // change
        {
            mousePos = Camera.main.ScreenToWorldPoint(Input.mousePosition);
