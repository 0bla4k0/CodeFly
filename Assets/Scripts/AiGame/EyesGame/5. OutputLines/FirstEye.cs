...
public class FirstEye : MonoBehaviour  // change script name
{
    ...
    void Start()
    {
        ...
        line = new GameObject("LineOutFirst").AddComponent<LineRenderer>(); // change line name
    ...
    void Update()
    {
        if (FirstItemSlot.SetInRightBoxFirstEye) // change
        {
            ...
            {
                if (MouseDragStart == false && OutputBlock.FConnectLineEnter == false) // change
                {
                    line.SetPosition(0, StartBoxBlock.transform.position);
        ...
        if (OutputBlock.FConnectLineEnter) // change
        {
            mousePos = Camera.main.ScreenToWorldPoint(Input.mousePosition);
