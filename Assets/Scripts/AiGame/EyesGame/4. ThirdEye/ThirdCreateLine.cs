...
public class ThirdCreateLine : MonoBehaviour // change script name
{
    ...
    void Start()
    {
        ...
        line = new GameObject("LineThird").AddComponent<LineRenderer>(); // change line name
    ...
    void Update()
    {
        if (ThirdItemSlot.SetInRightBoxThirdEye) // change
        {
            ...
            {
                if (MouseDragStart == false && ThirdConnectLine.ConnectLineEnter == false) // change
                {
                    line.SetPosition(0, StartBoxBlock.transform.position);
        ...
        if (ThirdConnectLine.ConnectLineEnter) // change
        {
            mousePos = Camera.main.ScreenToWorldPoint(Input.mousePosition);
